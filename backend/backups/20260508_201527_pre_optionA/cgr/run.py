...
def review_file(
    ...
    *,
    ...
    db = get_master_db()
    print(f"[마스터 DB] {db.path} ({len(db.all_articles())}개 조)", file=sys.stderr)
    ...
    if context is not None:
        ...
        print(f"[사업장 정보] {n_skipped}개 슬롯 SKIP (미적용)", file=sys.stderr)
    ...
    def _extract_article(article: int, slots):
        ...
        try:
            ...
            elapsed = time.time() - t0
            print(f"  [제{article}조] 추출 실패 ({elapsed:.1f}s): {e}", file=sys.stderr)
            return article, slots, [], e

    # 사전필터 — 사업장 본문에 부재한 영역의 조는 LLM 호출 안 함
    print(f"[사전필터] {len(by_art)}개 조 임베딩 유사도 검사...", file=sys.stderr)
    pf_t0 = time.time()
    try:
        ...
        print(f"[사전필터] 실패 — 모든 조 LLM 호출로 진행: {e}", file=sys.stderr)
    ...
    print("[선택 조 디스플레이] 임베딩 매칭 시작 (백그라운드)...", file=sys.stderr)
    ...
    print(f"[추출 시작] {len(active_by_art)}개 조 병렬 호출 (동시성 30)", file=sys.stderr)
    t0 = time.time()
    article_results: list[ArticleResult] = []
    with ThreadPoolExecutor(max_workers=min(30, len(active_by_art) or 1)) as ex:
        futures = [ex.submit(_extract_article, a, s) for a, s in sorted(active_by_art.items())]
        results = []
    ...
    print(f"[추출 완료] 총 {time.time() - t0:.1f}s", file=sys.stderr)
    ...
    print("[사유 풀이] LLM 호출 중...", file=sys.stderr)
    t0 = time.time()
    try:
        explain_findings(all_findings, slots_by_id)
        print(f"[사유 풀이] 완료 ({time.time() - t0:.1f}s)", file=sys.stderr)
    except Exception as e:
        print(f"[사유 풀이] 실패 — 기술적 사유 그대로 표시: {e}", file=sys.stderr)
    ...
    try:
        ...
        print(f"[선택 조 디스플레이] 임베딩 실패 — 빈 디스플레이로 진행: {e}", file=sys.stderr)
        optional_displays = []
    finally:
